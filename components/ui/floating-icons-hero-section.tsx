...
const Icon = ({
  ...
  return (
    <motion.div
      ...
      className={cn('absolute group', iconData.className)}
    >
      {/* Inner wrapper for the continuous floating animation */}
      {iconData.href ? (
        <a href={iconData.href} className="block" aria-label={iconData.label || undefined}>
          <motion.div
            className="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 p-3 rounded-3xl shadow-xl bg-card/80 backdrop-blur-md border border-card-border/30 cursor-pointer group-hover:scale-110 transition-transform duration-200"
            animate={{
              y: [0, -8, 0, 8, 0],
              x: [0, 6, 0, -6, 0],
              rotate: [0, 5, 0, -5, 0],
            ...
            whileTap={{ scale: 0.95 }}
          >
            <iconData.icon className="w-8 h-8 md:w-10 md:h-10 text-foreground group-hover:text-accent-color transition-colors duration-200" />
          </motion.div>
        </a>
      ) : (
        <motion.div
      ...
      {/* Tooltip */}
      {iconData.label && (
        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-card px-2.5 py-1 text-xs font-medium text-card-foreground shadow-md border border-card-border opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
          {iconData.label}
        </div>
      )}
    </motion.div>
